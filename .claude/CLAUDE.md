...
Proxo is a REST API (TypeScript/Hono) that lets AI agents purchase anything on the internet using USDC stablecoin. No API keys. No registration. The agent's `wallet_id` is its credential. Proxo auto-routes payments — x402-native merchants get paid directly (0.5% fee), everything else goes through Browserbase cloud browser checkout with browser-use (5% fee). Same interface, same receipt format either way.
...
- **Browser checkout**: Browserbase + browser-use (Claude Sonnet 4). 5% fee. Fresh session per checkout. Domain-level page caching for repeat flows.

### Credential Placeholder System

LLM never sees real card numbers. browser-use gets placeholders (`x_card_number`, `x_card_cvv`, etc.) which are injected into the DOM as `sensitive_data`. Real values come from `.env` and never enter the LLM context.
...
- **Browser Automation:** browser-use + Browserbase SDK
...
├── checkout/    # Browserbase sessions, browser-use, placeholders, domain cache
...
- **Shipping per-purchase** — custom shipping in buy request. Falls back to .env defaults. Returns `SHIPPING_REQUIRED` if missing for physical items.
...
- Agent data (shipping info) MUST be sanitized before injection into browser-use tasks
- browser-use receives deterministic task templates, NOT raw agent input
...
- See `plans/07-testing-guidelines.md` and `plans/13-phased-build-plan.md` for details
...
| `12-error-handling.md` | Error codes, HTTP statuses, recovery |
| `13-phased-build-plan.md` | 7 phases with test gates |
| `skill.md` | Agent-facing API quick reference |
