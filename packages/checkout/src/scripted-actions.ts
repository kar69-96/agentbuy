...
const CARD_FIELD_MAP: Array<{ selector: string; credKey: string }> = [
  ...
  {
    selector:
      'input[name*="exp" i], input[name*="encryptedExpiryDate" i], input[autocomplete="cc-exp"]',
  ...
  {
    selector:
      'input[name*="holderName" i], input[name*="cardholder" i], input[autocomplete="cc-name"]',
...
export async function scriptedDismissPopups(page: Page): Promise<string[]> {
  return page.evaluate(() => {
    ...
    document
      ...
      .forEach((e) => {
        e.remove();
        actions.push("removed cookie banner");
      });

    // Remove fixed overlays
    document
      .querySelectorAll(
        '.overlay, .backdrop, [class*="overlay" i], [class*="backdrop" i]',
      )
      .forEach((e) => {
        if (
          !CAPTCHA_RE.test(e.textContent || "") &&
...
export async function scriptedFillShipping(
  ...
  const filled = await page.evaluate((d) => {
    ...
    if (email) {
      fillInput(email as HTMLInputElement, d.email);
      results.push("email");
    }
    ...
    const fn = !fullName
      ...
      : null;
    if (fn) {
      fillInput(fn as HTMLInputElement, d.firstName);
    ...
    const ln = !fullName
      ...
      : null;
    if (ln) {
      fillInput(ln as HTMLInputElement, d.lastName);
    ...
    if (addr) {
      fillInput(addr as HTMLInputElement, d.street);
    ...
    if (apt && d.apartment) {
      fillInput(apt as HTMLInputElement, d.apartment);
    ...
    if (city) {
      fillInput(city as HTMLInputElement, d.city);
      results.push("city");
    }
    ...
    if (stateSelect) {
      if (fillSelect(stateSelect as HTMLSelectElement, d.state))
        results.push("state");
    } else {
      ...
      if (stateInput) {
        fillInput(stateInput as HTMLInputElement, d.state);
        results.push("state");
      }
    }
    ...
    if (zip) {
      fillInput(zip as HTMLInputElement, d.zip);
      results.push("zip");
    }
    ...
    if (countrySelect && d.country) {
      if (fillSelect(countrySelect as HTMLSelectElement, d.country))
    ...
    if (phone) {
      fillInput(phone as HTMLInputElement, d.phone);
      results.push("phone");
    }
...
export async function scriptedFillCardFields(
  ...
  async function fillWithTimeout(
    ...
    try {
      await Promise.race([
        ...
        new Promise<never>((_, reject) =>
          setTimeout(() => reject(new Error("fill timeout")), ms),
        ),
      ]);
      return true;
    } catch {
  ...
  if (expiry) {
    ...
    if (filledSplit) {
      for (const sel of EXPIRY_YEAR_SELECTORS) {
        const el = page.locator(sel).first();
        if (
          (await fillWithTimeout(el, fullYear)) ||
          (await fillWithTimeout(el, year))
        ) {
...
export async function scriptedFillBilling(
  ...
  return page.evaluate((d) => {
    ...
    const street = find([
      'input[name*="billing"][name*="address" i]',
      'input[name*="billing"][name*="street" i]',
      'input[id*="billing"][id*="address" i]',
    ]);
    if (street) {
      fillInput(street as HTMLInputElement, d.street);
      results.push("billing_street");
    }

    const city = find([
      'input[name*="billing"][name*="city" i]',
      'input[id*="billing"][id*="city" i]',
    ]);
    if (city) {
      fillInput(city as HTMLInputElement, d.city);
      results.push("billing_city");
    }

    const state = find([
      'select[name*="billing"][name*="state" i]',
      'select[name*="billing"][name*="zone" i]',
      'select[name*="billing"][name*="province" i]',
    ]);
    if (state) {
      if (fillSelect(state as HTMLSelectElement, d.state))
        results.push("billing_state");
    }

    const zip = find([
      'input[name*="billing"][name*="zip" i]',
      'input[name*="billing"][name*="postal" i]',
      'input[id*="billing"][id*="zip" i]',
    ]);
    if (zip) {
      fillInput(zip as HTMLInputElement, d.zip);
      results.push("billing_zip");
    }
...
export async function scriptedUncheckBillingSameAsShipping(
  page: Page,
): Promise<boolean> {
  return page.evaluate(() => {
...
export async function scriptedClickButton(
  ...
  const clicked = await page.evaluate((t) => {
    ...
    for (const el of candidates) {
      ...
      const match = alternatives.some(
        (alt) => allText.includes(alt) || wordsMatch(allText, alt),
      );
      if (!match) continue;
...
export async function detectPageType(page: Page): Promise<PageType> {
  const evalBody = ({ cardSelectors }: { cardSelectors: string[] }) => {
    ...
    const donationSignals = ["donate", "donation", "contribution", "give now"];
    const donationCount = donationSignals.filter((s) =>
      text.includes(s),
    ).length;
    ...
    if (
      (donationCount >= 2 && (hasDonateButton || hasDonationAmounts)) ||
      isDonationSite
    ) {
      // Check if this is actually the payment gateway (has card fields)
      const hasCardFields = cardSelectors.some(
    ...
    if (confirmCount >= 2 || (confirmCount >= 1 && isConfirmUrl)) {
      ...
      const errorOnConfirmCount = errorTextSignals.filter((s) =>
        text.includes(s),
      ).length;
    ...
    const shippingFieldCount = shippingSelectors.filter(
      (sel) => document.querySelector(sel) !== null,
    ).length;
    ...
    const verificationCount = verificationSignals.filter((s) =>
      text.includes(s),
    ).length;
    ...
    const emailOnlyInputs = document.querySelectorAll(
      'input[type="email"], input[name*="email" i]',
    );
    const totalFormInputs = document.querySelectorAll(
    ...
    // Registration form — distinguished from login by password confirmation or registration-specific URL/signals
    ...
    const registrationCount = registrationSignals.filter((s) =>
      text.includes(s),
    ).length;
    ...
    if (isRegistrationUrl && registrationCount >= 1) {
      return "registration-form" as const;
    }
    if (hasPasswordFields && registrationCount >= 1) {
      return "registration-form" as const;
    }
    ...
    const errorTextCount = errorTextSignals.filter((s) =>
      text.includes(s),
    ).length;
...
export async function extractConfirmationData(
  page: Page,
...
export async function extractVisibleTotal(
  page: Page,
...
export async function extractErrorMessage(page: Page): Promise<ErrorData> {
  return page.evaluate(() => {
    ...
    else if (cardInvalidPatterns.some((p) => text.includes(p)))
      type = "card_invalid";
    else if (outOfStockPatterns.some((p) => text.includes(p)))
      type = "out_of_stock";
    ...
    for (const sel of errorSelectors) {
      ...
      for (const el of els) {
        ...
        if (
          elText.length > 0 &&
          elText.length < 500 &&
          errorConfirming.some((p) => elText.toLowerCase().includes(p))
        ) {
...
export async function scriptedFillVerificationCode(
  ...
  return page.evaluate((c) => {
    ...
    if (
      splitInputs.length >= 4 &&
      splitInputs.length <= 8 &&
      c.length === splitInputs.length
    ) {
